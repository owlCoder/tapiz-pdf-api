...
// ------------------------------------------------------------------
// 1. Include a Unicode font (DejaVu Sans) as Base64
//    Replace the placeholder with the actual Base64 of DejaVuSans.ttf
// ------------------------------------------------------------------
const dejavuSansBase64 = "PLACEHOLDER_BASE64_STRING_OF_DEJAVU_SANS_TTF";

function registerUnicodeFont(doc) {
  // Add the font file to virtual file system
...
// ------------------------------------------------------------------
// 2. Helpers (no more srl() – we keep the original UTF-8 string)
// ------------------------------------------------------------------
...
function progressBar(doc, x, y, w, pct, required) {
  ...
  if (pct > 0)
    roundRect(doc, x, y, Math.max(4, (w * pct) / 100), h, 2.5, color);
  ...
  doc.text(`${required}% (prag)`, markerX, y + h + 4, { align: "center" });
  doc.setTextColor(...C.gray400);
  doc.text("100%", x + w, y + h + 4, { align: "right" });
}

function sessionBars(doc, pageW, y, sessions, total) {
  const margin = 19,
    chartW = pageW - margin * 2,
    maxH = 26,
    n = sessions.length;
  if (n === 0) return y;
  ...
  sessions.forEach((s, i) => {
    ...
    doc.setFont("DejaVuSans", "normal");
    doc.setTextColor(...C.gray500);
    doc.text(`T${s.sessionNumber}`, bx + barW / 2, y + maxH + 4.5, {
      align: "center",
    });
    if (bh > 6) {
      doc.setFontSize(5);
      doc.setTextColor(...C.white);
      doc.text(`${Math.round(p * 100)}%`, bx + barW / 2, by + bh - 1.5, {
        align: "center",
      });
    }
  });
...
function drawFooters(doc, pageW, label) {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.text(`Strana ${i} od ${total}`, pageW - 14, 290, { align: "right" });
  }
}
...
// ------------------------------------------------------------------
// 5. PDF endpoint: Statistike prisustva (stats)
// ------------------------------------------------------------------
app.post("/api/pdf/stats", async (req, res) => {
  try {
    ...
    if (!stats || !subject) {
      return res
        .status(400)
        .json({ error: "Missing required fields: stats, subject" });
    }
    ...
    const avgPct =
      stats.totalSessions > 0 && stats.perStudent.length > 0
        ? Math.round(
            stats.perStudent.reduce((acc, s) => acc + parseInt(s.count), 0) /
              stats.perStudent.length /
              stats.totalSessions *
              100
          )
        : 0;
    const passing = stats.perStudent.filter(
      (s) =>
        Math.round((parseInt(s.count) / stats.totalSessions) * 100) >=
        attendanceRequired
    ).length;
    ...
    y = kpiCards(doc, pageW, y, [
      ...
      { label: "Termina održano", value: String(stats.totalSessions), color: [14, 165, 233] },
      {
        label: "Prosečno prisustvo",
        value: `${avgPct}%`,
        color: avgPct >= attendanceRequired ? C.emerald : C.amber,
      },
      {
        label: `Ispunili ${attendanceRequired}%`,
        value: `${passing}/${stats.perStudent.length}`,
        color: passing === stats.perStudent.length ? C.emerald : C.red,
      },
    ]);
    ...
    doc.setFont("DejaVuSans", "bold");
    doc.setTextColor(...C.gray900);
    doc.text("Ukupno prisustvo", 19, y + 6.5);
    badge(
      doc,
      pageW - 70,
      y + 5,
    ...
    if (stats.perStudent.length > 0) {
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          0: { cellWidth: 52 },
          1: { cellWidth: 34, font: "DejaVuSans", fontSize: 7 },
        ...
        didParseCell: (data) => {
          ...
          if (data.column.index === 3 && data.section === "body") {
            ...
            data.cell.styles.textColor =
              p2 >= attendanceRequired ? C.emerald : p2 >= 50 ? C.amber : C.red;
          }
        },
    ...
    drawFooters(doc, pageW, `Evidentiraj · ${subject.name}`);

    const pdfBuffer = Buffer.from(doc.output("arraybuffer"));
    const filename = `statistike_${subject.code}_${new Date()
      .toISOString()
      .slice(0, 10)}.pdf`;
    ...
    return res
      .status(500)
      .json({ error: "Failed to generate PDF", details: String(err) });
  }
});

// ------------------------------------------------------------------
// 6. PDF endpoint: Bodovna lista (scoresheet)
// ------------------------------------------------------------------
app.post("/api/pdf/scoresheet", async (req, res) => {
  try {
    ...
    if (!name || !columns || !rows) {
      return res
        .status(400)
        .json({ error: "Missing fields: name, columns, rows are required" });
    }
    ...
    autoTable(doc, {
      ...
      startY: 38,
      theme: "grid",
      styles: { font: "DejaVuSans", fontSize: 8, cellPadding: 2, overflow: "linebreak" },
      headStyles: {
        fillColor: [23, 94, 141],
        textColor: 255,
        fontSize: 8,
    ...
    drawFooters(doc, pageW, `Bodovna lista · ${name}`);

    const pdfBuffer = Buffer.from(doc.output("arraybuffer"));
    const filename = `${name}_${academicYear || ""}.pdf`.replace(
      /[^a-z0-9_\- ]/gi,
      "_"
    );
...
// ------------------------------------------------------------------
// 7. PDF endpoint: Evidencija prisustva (attendances)
// ------------------------------------------------------------------
app.post("/api/pdf/attendances", async (req, res) => {
  try {
    ...
    if (!attendances || !subject) {
      return res
        .status(400)
        .json({ error: "Missing required fields: attendances, subject" });
    }
    ...
    attendances.forEach((a) => {
      sessionCounts[a.session.sessionNumber] =
        (sessionCounts[a.session.sessionNumber] ?? 0) + 1;
    });
    const sessionNums = Object.keys(sessionCounts)
      .map(Number)
      .sort((a, b) => a - b);
    const uniqueStudents = new Set(attendances.map((a) => a.student.id)).size;
    const avgPerSession =
      sessionNums.length > 0 ? Math.round(attendances.length / sessionNums.length) : 0;
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        0: { cellWidth: 55 },
        1: { cellWidth: 35, font: "DejaVuSans", fontSize: 7 },
    ...
    const pdfBuffer = Buffer.from(doc.output("arraybuffer"));
    const filename = `prisustva_${subject.code}_${new Date()
      .toISOString()
      .slice(0, 10)}.pdf`;
